...
### v1.1.0（2026-08-19）——当前版本

**新功能**：
- **Agent 预设汉化**：预设名称与说明显示中文（含「Agent预设汉化」开关，字典可在 settings.yaml 的 `gui-hanhua.presets` 段维护）
- **AI 自动识别 / AI 自检覆盖 Agent 预设**：自动扫描 `~/.dsh/.agent-presets/` 目录并汉化未汉化的预设
- **版本检查与自动更新**：启动/手动对比 GitHub 仓库版本（旧/当前/领先三态识别），发现新版本自动弹窗确认，确认后自动下载并覆盖插件文件（覆盖前自动备份 `.bak`）

**修复的 Bug**：
- 移除不兼容的 `z.object().partial()`（曾导致插件加载失败、dsh 启动崩溃）
- schema 构建增加兜底：任何失败自动退回宽松结构，插件永不再因自身问题崩溃
- AI 任务提示词中的硬编码用户路径通用化（隐私修复）

### v1.0.0（2026-08-18）

**新功能**：
- 工具卡片 / 插件列表 / 命令面板 / 权限选择器汉化
- 总开关 + 分项开关 + 配置持久化
- 翻译字典可视化编辑（搜索 / 增删 / 启用禁用 / 导入导出 / 恢复默认）
